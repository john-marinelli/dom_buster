...
def main(url_path, xpath_val=None, xpath_flag = False, list_val="res/xss-payload-list.txt", list_flag=False, test=False, speed_val=3):
    ...
    for i in range(num_inputs):
        #initial collecting of input fields before payload loop
        #along with xpath navigation
        if (xpath_flag):
            input_tester.pre_xpath(xpath)
            input_fields = input_tester.list_inputs()
        else:
        ...
        if input_fields[i].is_displayed() and input_fields[i].is_enabled():
            for payload in payloads:
                #Types payload to browser input fields, then submits. First grabs innerHTML of input box
                #for logging purposes
                id = input_fields[i].get_attribute("outerHTML")
                input_fields[i].send_keys(payload)
                input_fields[i].submit()
                ...
                input_tester.load_url(url)
                #Re-collect input fields (Some field ID's may be unique to individual sessions.)
                #Also navigates to specified xpath again
                if (xpath_flag):
                    input_tester.pre_xpath(xpath)
                    input_fields = input_tester.list_inputs()
                else:
    ...
    for arg, val in args:
        if arg in ("-h, --help"):
            ...
            try:
                ...
                print("Please enter a valid integer of value 1 or more for speed.")

    if list_flag == False:
        list_path = "res/xss-payload-list.txt"
